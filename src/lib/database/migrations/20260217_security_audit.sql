...
BEGIN
    ...
    v_is_admin := EXISTS (
        SELECT 1 FROM users WHERE id = v_viewer_id AND role IN ('admin', 'super_admin')
    );
...
-- B. Strict RLS for Submissions (if not already strictly enforced)
...
-- C. Strict RLS for Users (Hide emails/phones from public SELECT keys)
...
-- Note: This means 'supabase.from("users").select()' will fail for others. 
